...
export default function UseStprofileUpdate(submit) {
  ...
  useEffect(() => {
    if (initialState) {
      setvalues({
        ...values,
        firstName: initialState.user.first_name,
        lastName: initialState.user.last_name,
        userName: initialState.user.username,
        phoneNumber: initialState.user.phone_no,
        email: initialState.user.email,
        address: initialState.user.address,
        des: initialState.description,
      });
    }
  }, [initialState]);
